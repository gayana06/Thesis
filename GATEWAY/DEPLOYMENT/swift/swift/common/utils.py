...
from  swift.oracle_plus.quorum_handler import get_quorum
...
class GreenAsyncPile(object):
    ...
    def _run_func(self, func, args, kwargs):
	    ...
	    #with open("/home/ubuntu/spawn.txt", "a") as tran_file:
            #    tran_file.write("Inside _run_func Datetime="+str(datetime.now())+"  Args="+str(args)+"\n")
    ...
    #gayana-start
    def spawn_new(self, func, *args, **kwargs):
        """
        Spawn a job in a green thread on the pile.
        """
        self._inflight += 1
        #self._pool.spawn(self._run_func, func, args, kwargs)
        thread = Thread(target =self._run_func, args = (func, args, kwargs, ))
        thread.start()
    ...
    def next(self):
	    ...
	    if self._inflight == 0:
                #with open("/home/ubuntu/spawn.txt", "a") as tran_file:
                #    tran_file.write("Exiting at Datetime="+str(datetime.now())+"\n")
                raise StopIteration()
            else:
                #with open("/home/ubuntu/spawn.txt", "a") as tran_file:
                #    tran_file.write("Start waiting at Datetime="+str(datetime.now())+"\n")
                item=self._responses.get()
                #with open("/home/ubuntu/spawn.txt", "a") as tran_file:
                #   tran_file.write("Resuming at Datetime="+str(datetime.now())+"\n")
...
def new_quorum_size(req_path):
    ...
    :return:(R,W)
    """
    return get_quorum(req_path)
'''
    if 'test1' in req_path:
        return 1,5
    elif 'test2' in req_path:
        return 5,1
    else:
        return 3,3
'''
