...
char	LineBuffer[MAXLINEBUFFER], *LBp=LineBuffer, *attributeString;
...
Boolean MyReadName(String s, int n, char ColonOpt)
/*      --------  */
{
    register char *Sp=s;
    register int  c;
    ...
    while ( c != ColonOpt && c != ',' && c != '\n' && c != '|' && c != EOF )
    {
		...
		if ( c == '.' )
		{
			if ( (c = MyInChar()) == '|' || Space(c) || c == EOF ) break;
			*Sp++ = '.';
			continue;
		}
...
/*  ------  */
{
    if ( ! *LBp)
	{
		if( lineRead == 0){
			LBp = LineBuffer;
			strcpy(LineBuffer, attributeString);
			LineNo++;
			lineRead++;
		} else {
			LineBuffer[0] = '\00';
...
DataRec MyGetDataRec(Boolean Train)
/*      ----------  */
{
    Attribute	Att;
    char	Name[1000], *EndName;
    ...
    if ( MyReadName(Name, 1000, '\00') )
    {
		...
		ForEach(Att, 1, MaxAtt)
		{
			...
			if ( ! FirstValue && ! MyReadName(Name, 1000, '\00') )
			{
				XError(HITEOF, AttName[Att], "");
				FreeLastCase(DVec);
				return Nil;
...
/*   ------------  */
{
    free(&DVec[-1]);
    IValsOffset = 0;
    memset(LineBuffer,0,MAXLINEBUFFER);
}
