...
from swift.common.utils import normalize_timestamp, config_true_value, \
    ...
    quorum_size, new_quorum_size, GreenAsyncPile
...
from swift.oracle_plus.config import IS_READ_PARALLEL_MAX,REPLICA_COUNT,IS_PARALLEL_READ
...
class GetOrHeadHandler(object):
	...
	#gayana-start
        #with open("/home/ubuntu/PrintDetails.txt", "a") as tran_file:
        #    tran_file.write('App='+str(app)+'\nRing='+str(ring)+'\nServer Type='+str(server_type)+'\nPartition='+str(partition)+'\nPath='+str(path)+'\n')
        #    tran_file.write('Backend Header='+str(backend_headers)+'\nRequest Method='+str(req.method)+'\nRequest Path='+str(req.path)+'\n')
        #    tran_file.write('Request Query='+str(req.query_string)+'\nNewest='+str(self.newest)+'\nEnviron='+str(req.environ)+'\n\n\n')
        #gayana-end
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if IS_PARALLEL_READ:
                        new_source, new_node = self._get_source_and_node_parallel()
                    else:
    ...
    #gayana-still dev
    def _get_source_and_node_parallel(self):
        self.statuses = []
        self.reasons = []
        self.bodies = []
        self.source_headers = []
        ...
        #    tran_file.write("Enter at =  "+str(datetime.now())+"\n")
        start=time.time()
        node_timeout = self.app.node_timeout
        if self.server_type == 'Object' and not self.newest:
            node_timeout = self.app.recoverable_node_timeout

        def do_parallel_reads(node):
            start_node_timing = time.time()
            try:
                #with open("/home/ubuntu/spawn-read.txt", "a") as tran_file:
                #    tran_file.write("IP="+str(node['ip'])+" Port="+str(node['port'])+" Device="+str(node['device'])+" Path="+str(self.path)+" Datetime="+str(datetime.now())+" Method="+str(self.req_method)+"\n")
        ...
        if IS_READ_PARALLEL_MAX:
            parallel_read_count = REPLICA_COUNT
        else:
            readQ,writeQ=new_quorum_size(str(self.req_path))
            parallel_read_count = readQ
        #with open("/home/ubuntu/spawn-read.txt", "a") as tran_file:
        #    tran_file.write("parallel_read_count =  "+str(parallel_read_count)+"\n")
        pile = GreenAsyncPile(parallel_read_count)
        count=0
        for node in self.app.iter_nodes(self.ring, self.partition):
            if node in self.used_nodes:
                continue
            if count < parallel_read_count:
                pile.spawn(do_parallel_reads,node)
            else:
                break
            count+=1
        #with open("/home/ubuntu/spawn-read.txt", "a") as tran_file:
        #    tran_file.write("Waiting at "+str(datetime.now())+"\n")
        for possible_source in pile:
            #with open("/home/ubuntu/spawn-read.txt", "a") as tran_file:
            #    tran_file.write("Possible source at "+str(datetime.now())+"\n")
            ...
            if self.is_good_source(possible_source):
                ...
                if not float(possible_source.getheader('X-PUT-Timestamp', 1)):
                    ...
                    self.bodies.append('')
                    self.source_headers.append('')
                    sources.append((possible_source, possible_source.node))
                    #if not self.newest and not get_oracle().run_adaptive():  # one good source is enough
                    #    break
                    if oracle.get_oracle().run_adaptive():
                        if len(sources) >= oracle.my_quorum_size('GET'):
                            break
                    #gayana-start
                    else:
                        readQ,writeQ=new_quorum_size(str(self.req_path))
                        #with open("/home/ubuntu/spawn-read.txt", "a") as tran_file:
                        #    tran_file.write("Quorum Check for node = "+str(node['ip'])+" at "+str(datetime.now())+"\n")
                        if len(sources) >= readQ:
                            break
                    #gayana-end
            else:
                self.statuses.append(possible_source.status)
        ...
        if sources:
            sources.sort(key=lambda s: source_key(s[0])) #order by timestamp
            source, node = sources.pop()
            with open("/home/ubuntu/ReadA.txt", "a") as tran_file:
                tran_file.write("Time to Response obj = "+str(self.req_path)+"-"+str(time.time()-start)+"-"+str(datetime.now())+"\n")
            for src, _junk in sources:
                close_swift_conn(src)
            if not oracle.get_oracle().run_adaptive():
    ...
    def _get_source_and_node(self):
        ...
        sources = []
        #with open("/home/ubuntu/spawn-read.txt", "a") as tran_file:
        #    tran_file.write("Enter at normal =  "+str(datetime.now())+"\n")
        start = time.time()
        node_timeout = self.app.node_timeout
        if self.server_type == 'Object' and not self.newest:
            node_timeout = self.app.recoverable_node_timeout
        ...
        if sources:
            ...
            source, node = sources.pop()
            with open("/home/ubuntu/spawn-read.txt", "a") as tran_file:
                #tran_file.write("Source selected at =  "+str(datetime.now())+"\n")
                tran_file.write("Total - time : "+str(time.time()-start)+"\n")
            for src, _junk in sources:
                close_swift_conn(src)
            if not oracle.get_oracle().run_adaptive():
                self.used_nodes.append(node)
    ...
    def get_working_response(self, req):
        if IS_PARALLEL_READ:
            source, node = self._get_source_and_node_parallel()
        else:
        ...
        if source:
            ...
            if req.method == 'GET' and \
                ...
                get_quorum_handler().check_tail_read(str(req.path),res.body)
...
class Controller(object):
    ...
    def make_requests(self, req, ring, part, method, path, headers,
        ...
        for resp in pile:
            ...
            #    break
            if self.new_have_quorum(statuses,method,str(req.path)):
                break
    ...
    def my_make_requests(self, req, ring, part, method, path, headers,
                      query_string=''):
        """
        Sends an HTTP request to multiple nodes and aggregates the results.
        It attempts the primary nodes concurrently, then iterates over the
        ...
        for resp in pile:
            ...
            statuses.append(resp[0])
            if method == 'POST':
                self.app.logger.warn(_('--- Maria quorum de elementos my_make_requests ' + str(len(start_nodes))))
            if self.my_have_quorum(statuses, method):
            #if self.have_quorum(statuses, len(start_nodes)):
                break
        # give any pending requests *some* chance to finish
        pile.waitall(self.app.post_quorum_timeout)
        ...
        statuses, reasons, resp_headers, bodies = zip(*response)
        if oracle.get_oracle().run_adaptive():
            return self.my_best_response(req, statuses, reasons, bodies,
                                  '%s %s' % (self.server_type, req.method), method,
                                  headers=resp_headers)
        else:
            return self.best_response(req, statuses, reasons, bodies,
                                  '%s %s' % (self.server_type, req.method),
    ...
    #gayana-start
    def new_have_quorum(self, statuses, request_type, req_path,w_quorum=None):
        """
        Given a list of statuses from several requests, determine if
        a quorum response can already be decided.
        ...
        if w_quorum:
            ...
            read_q,write_q = new_quorum_size(req_path)
            if request_type=='GET':
                quorum=read_q
            else:
                quorum=write_q
    ...
    def my_have_quorum(self, statuses, request_type):
        """
        Given a list of statuses from several requests, determine if
        a quorum response can already be decided.
        ...
        :returns: True or False, depending on if quorum is established
        """
        quorum = oracle.my_quorum_size(request_type)
    ...
    def best_response(self, req, statuses, reasons, bodies, server_type,
        ...
        if len(statuses):
            for hundred in (HTTP_OK, HTTP_MULTIPLE_CHOICES, HTTP_BAD_REQUEST):
                ...
                #gayana-start
                readq,writeq=new_quorum_size(str(req.path))
                quorumval=0
    ...
    def my_best_response(self, req, statuses, reasons, bodies, server_type, request_type,
                      etag=None, headers=None):
        """
        Given a list of responses from several servers, choose the best to
        return to the API.
        ...
        :param bodies: bodies of each response
        :param server_type: type of server the responses came from
        :param request_type: specifies the type of the original request
        :param etag: etag
        :param headers: headers of each response
        :returns: swob.Response object with the correct status, body, etc. set
        """
        ...
        if len(statuses):
            for hundred in (HTTP_OK, HTTP_MULTIPLE_CHOICES, HTTP_BAD_REQUEST):
                ...
                if request_type == 'POST':
                    self.app.logger.warn(_('--- Maria quorum de elementos  my_best_response' + str(len(statuses))))
                if len(hstatuses) >= oracle.my_quorum_size(request_type):
                    status = max(hstatuses)
                    status_index = statuses.index(status)
                    resp.status = '%s %s' % (status, reasons[status_index])
                    resp.body = bodies[status_index]
    ...
    def my_GETorHEAD_base(self, req, server_type, ring, partition, path):
        """
        Base handler for HTTP GET or HEAD requests.
        ...
        if not res:
            ...
            if oracle.get_oracle().run_adaptive():
                res = self.my_best_response(
                    req, handler.statuses, handler.reasons, handler.bodies,
                    '%s %s' % (server_type, req.method), 'GET',
                    headers=handler.source_headers)
            else:
                res = self.best_response(
                    req, handler.statuses, handler.reasons, handler.bodies,
