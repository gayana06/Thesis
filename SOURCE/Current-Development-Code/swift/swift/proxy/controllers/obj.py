...
from swift.common.utils import ContextPool, normalize_timestamp, \
    ...
    quorum_size, new_quorum_size, GreenAsyncPile, normalize_delete_at_timestamp
...
from swift.oracle_plus.config import IS_CLASSIC_WRITE

from swift.oracle import oracle
from swift.oracle import replicareconciliation
from  swift.oracle_plus import pending_request_monitor
...
class ObjectController(Controller):
    ...
    def GETorHEAD(self, req):
        ...
        if oracle.get_oracle().run_adaptive():
            clock = time.time()
        start = time.time()
        ...
        if oracle.get_oracle().run_adaptive():
            finish = time.time() - clock
            oracle.get_metrics_gatherer().add_get_duration(finish)
        end = time.time()
        #gayana-start
        duration=end-start
        #get_statistics().sum_get_duration(str(req.path),duration)
        get_statistics().sum_get_duration_new(str(req.path),duration)
	...
	with open("/home/ubuntu/Q.txt", "a") as tran_file:
                    tran_file.write("Out side  :"+str(req.method)+"\n")
        if self.app.object_post_as_copy:
            with open("/home/ubuntu/Q.txt", "a") as tran_file:
                    tran_file.write("Came here :"+str(req.method)+"\n")
	    req.method = 'PUT'
            ...
            if oracle.get_oracle().run_adaptive():
                resp = self.my_make_requests(req, self.app.object_ring, partition,
            ...
            if oracle.get_oracle().run_adaptive():
                finish = time.time() - clock
                oracle.get_metrics_gatherer().add_put_duration(finish)
            end=time.time()
            #gayana-start
            duration=end-start
            #get_statistics().sum_put_duration(str(req.path),duration)
            get_statistics().sum_put_duration_new(str(req.path),duration)
            #gayana-end
            return resp
    ...
    nodelock=Lock()
    timeoutval=5
    suspect_list={'172.31.0.115'}

    def releaseDelayNodes(self,nodeip):
        #with ObjectController.nodelock:
        if nodeip in self.delay_nodes.keys():
            del self.delay_nodes[nodeip]

    def IsPossibleNode(self,nodeip,path):
        #with ObjectController.nodelock:
        Rquorumsize,Wquorumsize=new_quorum_size(path)
        if Wquorumsize  == 5:
	    return True
        if nodeip not in self.delay_nodes.keys():
            self.delay_nodes[nodeip]=time.time()
            return True
	if nodeip in self.suspect_list:
            with ObjectController.nodelock:
	        rec=self.delay_nodes.get(nodeip)
                if rec:
                    diff=time.time()-rec
                    if(diff>self.timeoutval):
                        del self.delay_nodes[nodeip]
                        self.delay_nodes[nodeip]=time.time()
                        return True
                return False
	...
	#    tran_file.write("Nodes="+str(nodes)+"\n")
        for node in nodes:
            try:
		#with open("/home/ubuntu/spawn.txt", "a") as tran_file:
                #    tran_file.write("IP="+str(node['ip'])+"\nPort="+str(node['port'])+"\nDevice="+str(node['device'])+"\nPart="+str(part)+"\nPath="+str(path)+"\nHeaders="+str(headers)+"\n\n")
                #gayana-start
                #nodeip=node['ip']
                #if not self.IsPossibleNode(nodeip,path):
                #    continue
                #gayana-end
		start_time = time.time()
                with ConnectionTimeout(self.app.conn_timeout):
                    conn = http_connect(
                        ...
                        path, headers)
		#with open("/home/ubuntu/spawn.txt", "a") as tran_file:
            	#    tran_file.write("Returned Connection at ="+str(datetime.now())+"\n")
                self.app.set_node_timing(node, time.time() - start_time)
                with Timeout(self.app.node_timeout):
                    resp = conn.getexpect()
		    #resp=200
		    #with open("/home/ubuntu/spawn.txt", "a") as tran_file:
                    #    tran_file.write("Got resp ="+str(resp) +" at ="+str(datetime.now())+"\n")
                #if True:
		#    conn.resp = None
                #    conn.node = node
                #    return conn		
    ...
    def terminate_thread(self,thread):
        ...
        if not thread.isAlive():
            with open("/home/ubuntu/spawn.txt", "a") as tran_file:
                tran_file.write("Already Terminate Successful\n")
	    return
        ...
        if res == 0:
	    with open("/home/ubuntu/spawn.txt", "a") as tran_file:
                tran_file.write("Nonexistent thread\n")
            raise ValueError("nonexistent thread id")
        elif res > 1:
            # """if it returns a number greater than one, you're in trouble,
            # and you should call it again with exc=NULL to revert the effect"""
	    with open("/home/ubuntu/spawn.txt", "a") as tran_file:
                tran_file.write("Failed\n")
            ctypes.pythonapi.PyThreadState_SetAsyncExc(thread.ident, None)
            raise SystemError("PyThreadState_SetAsyncExc failed")
	else:
	    with open("/home/ubuntu/spawn.txt", "a") as tran_file:
            	tran_file.write("Terminate Successful\n")

    def _get_put_responses_new(self, req, conns, nodes):
        statuses = []
        reasons = []
        bodies = []
        etags = set()

        def get_conn_response_new(conn):
            try:
                with Timeout(self.app.node_timeout):
                    if conn.resp:
                        val=conn.resp
                        pile.append(val)
                        #self.releaseDelayNodes(conn.node['ip'])
                        #return val
                    else:
                        val=conn.getresponse()
			with open("/home/ubuntu/spawn.txt", "a") as tran_file:
                            tran_file.write("Got--- response at "+str(datetime.now())+" from node "+str(conn.node['ip'])+"\n")
                        pile.append(val)
                        #self.releaseDelayNodes(conn.node['ip'])
                        #return val
            except (Exception, Timeout):
        ...
        if oracle.get_oracle().run_adaptive():
            pile_responses = []
        #with open("/home/ubuntu/spawn.txt", "a") as tran_file:
            #tran_file.write("Before spawn\n")
	threads=[]
        for conn in conns:
            #pile.spawn(get_conn_response, conn)
            thread = Thread(target = get_conn_response_new, args = (conn, ))
            threads.append(thread)
	    thread.start()
        #with open("/home/ubuntu/spawn.txt", "a") as tran_file:
        #    tran_file.write("After spawn\n")
        #i=0
        #for r in pile:
        #    with open("/home/ubuntu/responses.txt", "a") as tran_file:
        #            tran_file.write("Response "+str(i) +"="+str(r) +"\n")
        #    i=i+1
        #i=0
        while True:
            if len(pile)>=3:
	...
	for t in threads:
        ...
        for response in pile:
            #with open("/home/ubuntu/responses.txt", "a") as tran_file:
            #        tran_file.write("Went through "+str(i) +"="+str(r) +"\n")
            #with open("/home/ubuntu/spawn.txt", "a") as tran_file:
            #tran_file.write("Inside for")
            if oracle.get_oracle().run_adaptive():
                pile_responses.append(response)
            if response:
                ...
                if oracle.get_oracle().run_adaptive():
                    if self.my_have_quorum(statuses, 'PUT'):
                        break
                else:
                    #gayana-start
                    ...
                    #    break
                    #time.sleep(1)
                    if self.new_have_quorum(statuses, 'PUT',req.path):
                        break
                    #gayana-end
        # give any pending requests *some* chance to finish
        #with open("/home/ubuntu/spawn.txt", "a") as tran_file:
            #tran_file.write("Just before final")
        #final_responses = pile.waitall(self.app.post_quorum_timeout)

        if oracle.get_oracle().run_adaptive():
            pile_responses += final_responses
            aux_pile = itertools.chain(pile_responses, pile)
        while len(statuses) < len(nodes):
            statuses.append(HTTP_SERVICE_UNAVAILABLE)
            reasons.append('')
            bodies.append('')
        if oracle.get_oracle().run_adaptive():
            replicareconciliation.get_replica_reconciliation().add_new_request(aux_pile)
            pending_request_monitor.get_pending_req_monitor().add_new_request(aux_pile)
    ...
    def _get_put_responses(self, req, conns, nodes, min_conns):
        statuses = []
        reasons = []
        bodies = []
        etags = set()
        ...
        def get_conn_response(conn):
            try:
                with Timeout(self.app.node_timeout):
                    #with open("/home/ubuntu/Writes.txt", "a") as tran_file:
                    #    tran_file.write("At Response start Datetime="+str(datetime.now())+"\n")
                    if conn.resp:
                        val=conn.resp
                    else:
                        val=conn.getresponse()
                    #with open("/home/ubuntu/Writes.txt", "a") as tran_file:
                    #    tran_file.write("At Response return Datetime="+str(datetime.now())+"\n")
                    return val
            except (Exception, Timeout):
                self.app.exception_occurred(
                    conn.node, _('Object'),
        ...
        #with open("/home/ubuntu/Writes.txt", "a") as tran_file:
        #    tran_file.write("At D Datetime="+str(datetime.now())+"\n")
        l=time.time()
        for response in pile:
            #with open("/home/ubuntu/Writes.txt", "a") as tran_file:
            #    tran_file.write("At inside pile Datetime="+str(datetime.now())+"\n")
            ...
            if response:
                ...
                if oracle.get_oracle().run_adaptive():
                    ...
                    #    break
		    #time.sleep(1)
                    if self.new_have_quorum(statuses, 'PUT',str(req.path), w_quorum=min_conns):
                        #with open("/home/ubuntu/Writes.txt", "a") as tran_file:
                        #    tran_file.write("Quorum Satisfied, Min-Conn = "+str(min_conns)+"  at "+str(datetime.now()) +"\n")
                        break
			#with open("/home/ubuntu/spawn.txt", "a") as tran_file:
                	#    tran_file.write("No break DateTime="+str(datetime.now())+"\n")
                    #gayana-end
        # give any pending requests *some* chance to finish
        #with open("/home/ubuntu/spawn.txt", "a") as tran_file:
        #    tran_file.write("Just before final Datetime= "+str(datetime.now())+"\n\n")
        m=time.time()-l
        #with open("/home/ubuntu/WritesE.txt", "a") as tran_file:
        #    tran_file.write("Wait Time to Response obj = "+str(req.path)+"="+str(m)+"="+str(datetime.now())+"\n")
        final_responses = pile.waitall(self.app.post_quorum_timeout)
	#for conn in conns:
        #    conn.close()
    ...
    def PUT(self, req):
        ...
        #with open("/home/ubuntu/Writes.txt", "a") as tran_file:
        #    tran_file.write("Inside PUT :"+str(req.method)+" at "+str(datetime.now())+"\n")
        if oracle.get_oracle().run_adaptive():
            clock = time.time()
        start=time.time()
        ...
        body=str(req.environ['wsgi.input'].read(request_body_size))
        rq,wq=new_quorum_size(req.path)
        body=body.split("|")[0]+"|"+str(wq)
        req.environ['wsgi.input'] = StringIO(body)
        req.environ['CONTENT_LENGTH'] = str(len(body))
        request_body_size = int(req.environ.get('CONTENT_LENGTH', 0))
        #with open("/home/ubuntu/Inject.txt", "a") as tran_file:
        #    tran_file.write("Content Length="+str(request_body_size)+"\n")
        #    tran_file.write("Content ="+str(body)+"\n")
        ...
        if 'x-delete-at' in req.headers:
            ...
            delete_at_container = delete_at_part = delete_at_nodes = None
        #with open("/home/ubuntu/spawn.txt", "a") as tran_file:
        #    tran_file.write("\n\nAt A Datetime="+str(datetime.now())+"\n")
        ...
        connection_count=0
        if oracle.get_oracle().run_adaptive():
            min_conns = oracle.my_quorum_size('PUT')
        else:
            #min_conns = quorum_size(len(nodes))
	    #gayana-start
            readq,min_conns=new_quorum_size(req.path)
            #with open("/home/ubuntu/Quorum.txt", "a") as tran_file:
            #                tran_file.write("Write Quorum = "+str(min_conns)+" and req_path="+str(req.path)+"\n")
            #min_conns = quorum_size(len(nodes))
            #gayana-end

        if IS_CLASSIC_WRITE:
            connection_count=min_conns
        else:
            connection_count=len(nodes)

        #with open("/home/ubuntu/Writes.txt", "a") as tran_file:
        #    tran_file.write("Min-Conns = "+str(min_conns)+" and Connection count ="+str(connection_count)+" at "+str(datetime.now()) +"\n")

        #gayana-dev

        pile = GreenPile(connection_count)
        te = req.headers.get('transfer-encoding', '')
        chunked = ('chunked' in te)

        #with open("/home/ubuntu/spawn.txt", "a") as tran_file:
        #    tran_file.write("At B Datetime="+str(datetime.now())+"\n")
        outgoing_headers = self._backend_requests(
            req, connection_count, container_partition, containers,
            delete_at_container, delete_at_part, delete_at_nodes)

        #with open("/home/ubuntu/spawn.txt", "a") as tran_file:
        #    tran_file.write("At C Datetime="+str(datetime.now())+"\n")
        c=time.time()
        for nheaders in outgoing_headers:
            # RFC2616:8.2.3 disallows 100-continue without a body
            #with open("/home/ubuntu/Writes.txt", "a") as tran_file:
            #	tran_file.write("Node Header Datetime="+str(datetime.now())+"Header = "+str(nheaders)+"\n")
        ...
        nodes_ips=""
        for c in conns:
            nodes_ips+=str(c.node)+"|"
        #with open("/home/ubuntu/WritesC.txt", "a") as tran_file:
        #    tran_file.write("Connect PUT duration of obj = "+str(req.path)+"="+str(d)+"="+str(datetime.now())+" IPS = "+nodes_ips+"\n")
        ...
        if len(conns) < min_conns:
            self.app.logger.error(
                _('Object PUT returning 503-eee, %(conns)s/%(nodes)s '
                  'required connections'),
                {'conns': len(conns), 'nodes': min_conns})
            return HTTPServiceUnavailable(request=req)
        bytes_transferred = 0
        #with open("/home/ubuntu/Writes.txt", "a") as tran_file:
        #    tran_file.write("At A Datetime="+str(datetime.now())+"\n")
        e=time.time()
        try:
            ...
            f=time.time()-e
            #with open("/home/ubuntu/WritesD.txt", "a") as tran_file:
            #    tran_file.write(" SEND PUT obj = "+str(req.path)+"="+str(f)+"="+str(datetime.now())+"\n")
        ...
        statuses, reasons, bodies, etags = self._get_put_responses(req, conns,
                                                                   nodes,min_conns)
        b=time.time()-a;
        #with open("/home/ubuntu/WritesB.txt", "a") as tran_file:
        #    tran_file.write("Response PUT obj = "+str(req.path)+"="+str(b)+"="+str(datetime.now())+"\n")
        #with open("/home/ubuntu/spawn.txt", "a") as tran_file:
        #    tran_file.write("At J Datetime="+str(datetime.now())+"\n")
        if len(etags) > 1:
            self.app.logger.error(
        ...
        if oracle.get_oracle().run_adaptive():
            resp = self.my_best_response(req, statuses, reasons, bodies,
                                  _('Object PUT'), 'PUT', etag=etag)
        else:
        ...
        get_statistics().sum_put_duration_new(str(req.path),duration)
        #gayana-end
        #with open("/home/ubuntu/WritesA.txt", "a") as tran_file:
        #    tran_file.write("TOTAL PUT duration of obj = "+str(req.path)+"="+str(duration)+"="+str(datetime.now())+"\n")
        return resp
    ...
    def DELETE(self, req):
        ...
        headers = self._backend_requests(
            req, len(nodes), container_partition, containers)
        if oracle.get_oracle().run_adaptive():
            resp = self.my_make_requests(req, self.app.object_ring,
