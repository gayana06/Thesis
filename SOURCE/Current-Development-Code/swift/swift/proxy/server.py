...
from swift.oracle.oracle import get_oracle, get_metrics_gatherer
from swift.oracle_plus.oracle_plus import get_oracle_plus
...
class Application(object):
    ...
    def __init__(self, conf, memcache=None, logger=None, account_ring=None,
        ...
        self.conn_timeout = float(conf.get('conn_timeout', 1))
        self.client_timeout = int(conf.get('client_timeout', 60))
        self.put_queue_depth = int(conf.get('put_queue_depth', 10))
        self.object_chunk_size = int(conf.get('object_chunk_size', 65536))
        self.client_chunk_size = int(conf.get('client_chunk_size', 65536))
        self.trans_id_suffix = conf.get('trans_id_suffix', '')
        self.post_quorum_timeout = float(conf.get('post_quorum_timeout', 0.1))
    ...
    def __call__(self, env, start_response):
        ...
        get_oracle().run_oracle()

        get_oracle_plus().run_oracle_plus()
    ...
    def handle_request(self, req):
        ...
        get_statistics().add_request_object_to_map(req)
	    ...
	    #with open("/home/ubuntu/PrintRequest.txt", "a") as tran_file:
            #    tran_file.write("Request Method="+str(req.method)+"\nRequest Path="+str(req.path)+"\nRequest Environ="+str(req.environ)+"\n\n\n")
            if str(req.method) == 'GET' or str(req.method) == 'HEAD':
                get_metrics_gatherer().increase_gets()
            else:
                get_metrics_gatherer().increase_puts()
            get_statistics().add_objects_to_list(str(req.path_info),str(req.method))
